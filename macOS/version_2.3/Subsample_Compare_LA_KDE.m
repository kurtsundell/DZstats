function varargout = Subsample_Compare_LA_KDE(varargin)
% SUBSAMPLE_COMPARE_LA_KDE M-file for Subsample_Compare_LA_KDE.fig
%      SUBSAMPLE_COMPARE_LA_KDE, by itself, creates a new SUBSAMPLE_COMPARE_LA_KDE or raises the existing
%      singleton*.
%
%      H = SUBSAMPLE_COMPARE_LA_KDE returns the handle to a new SUBSAMPLE_COMPARE_LA_KDE or the handle to
%      the existing singleton*.
%
%      SUBSAMPLE_COMPARE_LA_KDE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SUBSAMPLE_COMPARE_LA_KDE.M with the given input arguments.
%
%      SUBSAMPLE_COMPARE_LA_KDE('Property','Value',...) creates a new SUBSAMPLE_COMPARE_LA_KDE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Subsample_Compare_LA_KDE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Subsample_Compare_LA_KDE_OpeningFcn via varargin.
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Subsample_Compare_LA_KDE_OpeningFcn, ...
                   'gui_OutputFcn',  @Subsample_Compare_LA_KDE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Subsample_Compare_LA_KDE is made visible.
function Subsample_Compare_LA_KDE_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Subsample_Compare_LA_KDE (see VARARGIN)
% Choose default command line output for Subsample_Compare_LA_KDE
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);
function varargout = Subsample_Compare_LA_KDE_OutputFcn(hObject, eventdata, handles) 
...
x = a:c:b;
rand_out = zeros(nsubsamples,nsamples);
for i = (1:nsamples);
for j = (1:ttrials);
datai = data(:,(i*2-1));
...
maxi = max(trimi);
trimi = data(1:maxi,(i*2-1));
randsubsampi = randsamp(trimi,nsubsamples);
rand_out(:,(i*2-1),j) = (randsubsampi);
randsubsampi = randsamp(datai,nsubsamples);
rand_out(:,i,j) = (randsubsampi);
end
end
...
for j = (1:ttrials);
for i = (1:nsamples);
rand_pdp_out(:,i,j) = ssvkernel(rand_out(:,i,j),x);
end
end
for j = (1:ttrials);
for i = (1:nsamples);
