function varargout = Intersample_Compare_LA_KDE(varargin)
% INTERSAMPLE_COMPARE_LA_KDE M-file for Intersample_Compare_LA_KDE.fig
%      INTERSAMPLE_COMPARE_LA_KDE, by itself, creates a new INTERSAMPLE_COMPARE_LA_KDE or raises the existing
%      singleton*.
%
%      H = INTERSAMPLE_COMPARE_LA_KDE returns the handle to a new INTERSAMPLE_COMPARE_LA_KDE or the handle to
%      the existing singleton*.
%
%      INTERSAMPLE_COMPARE_LA_KDE('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in INTERSAMPLE_COMPARE_LA_KDE.M with the given input arguments.
%
%      INTERSAMPLE_COMPARE_LA_KDE('Property','Value',...) creates a new INTERSAMPLE_COMPARE_LA_KDE or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Intersample_Compare_LA_KDE_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Intersample_Compare_LA_KDE_OpeningFcn via varargin.
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Intersample_Compare_LA_KDE_OpeningFcn, ...
                   'gui_OutputFcn',  @Intersample_Compare_LA_KDE_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Intersample_Compare_LA_KDE is made visible.
function Intersample_Compare_LA_KDE_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Intersample_Compare_LA_KDE (see VARARGIN)
% Choose default command line output for Intersample_Compare_LA_KDE
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% --- Outputs from this function are returned to the command line.
function varargout = Intersample_Compare_LA_KDE_OutputFcn(hObject, eventdata, handles) 
...
optw_out=zeros(length(x),nsamples+1);
for i = 1:nsamples;
mi = data(:,i*2-1);
mi = mi(isfinite(mi(:,1)),:);
mi(mi==0) = [];
[pdpi,~,optwi] = ssvkernel(mi,x);
optwi=transpose(optwi);
optw_out(:,1)=x;
optw_out(:,i+1) = optwi;
pdp_out(:,i+1) = pdpi;
pdp_out(:,1) = x;
...
optw_out1 = num2str(optw_out);
set(handles.listbox1,'String',optw_out1);
handles.pdp_out=pdp_out;
handles.pdp_cdf_out=pdp_cdf_out;
handles.bandwidth_out=optw_out;
handles.nsamples=nsamples;
guidata(hObject,handles);
